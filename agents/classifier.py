...
_CLASSIFY_PROMPT = """你是一个意图分类器。根据用户输入，将其分为以下五类之一：
...
- general：不属于以上四类的一般性对话、闲聊、方法建议、概念解释等

只输出类别标签（qa/writing/polish/arxiv/general），不要输出其他内容。

用户输入：{query}"""

_VALID_INTENTS = {"qa", "writing", "polish", "arxiv", "general"}
...
def classify_intent(state: dict, llm) -> dict:
    ...
    match = re.search(r"\b(qa|writing|polish|arxiv|general)\b", raw)
